...
def sample_without_replacement(factor_space: spaces.Space, n: int):
    ...
    if isinstance(factor_space, spaces.Box):
        return np.stack([np.atleast_1d(factor_space.sample()) for i in range(n)])
    elif isinstance(factor_space, spaces.Discrete):
        values = np.arange(factor_space.start, factor_space.start + factor_space.n)
...
def dict_to_list_of_dicts(d: Dict[str, np.ndarray]) -> List[Dict[str, np.ndarray]]:
    ...
    return [{key: d[key][i] for key in d} for i in range(lengths[0])]
...
class FactorWrapper:
    ...
    def reset(self, force_randomize_factor: bool = False):
        ...
        if force_randomize_factor or (
            self._num_resets_per_randomize
            and self._num_resets % self._num_resets_per_randomize == 0
        ):
            self.randomize_factor()
