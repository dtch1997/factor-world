...
class ObjectTextureWrapper(FactorWrapper):
    ...
    def _set_to_factor(self, value: int):
        ...
        # TODO: figure out why block only changes shades of red, black...
        mat_names = ["block_col", "block_wood", "block_red"]
        if hasattr(self, "object_name"):
            mat_names += f"material_0_{self.object_name}"

        texture_name = self.texture_names[value]
        texture = self.model.texture(texture_name)

        for mat_name in mat_names:
            mat = self.model.material(mat_name)
